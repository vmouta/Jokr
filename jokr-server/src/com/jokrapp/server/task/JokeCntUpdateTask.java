...
@Singleton
public class JokeCntUpdateTask {

	private static final Logger log = Logger
			.getLogger(JokeCntUpdateTask.class.getName());
	...
	public void execute() {
		...
		ArrayList<LinkedTreeMap<String, Object>> view = (ArrayList) db
				.view("query/jokesCnt").reduce(true).group(true)
		...
		for (LinkedTreeMap<String, Object> ht : view) {
			...
			try { // check if joke exists
				...
				if (user.getJokesCnt() != cnt.intValue()) {
					user.setJokesCnt(cnt.intValue());
					db.update(user);
				}
